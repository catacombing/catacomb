...
use smithay::utils::{IsAlive, Logical, Serial, Size, SERIAL_COUNTER};
use smithay::wayland::compositor;

use crate::protocols::session_lock::{SessionLockHandler, SessionLockManagerState};
...
impl LockSurfaceAttributes {
    fn ack_configure(&mut self, serial: Serial) -> Option<LockSurfaceConfigure> {
        let configure =
            self.pending_configures.iter().find(|configure| configure.serial == serial).cloned()?;
...
impl LockSurface {
    pub fn new(surface: WlSurface, shell_surface: ExtSessionLockSurfaceV1) -> Self {
        Self { surface, shell_surface }
    }
    ...
    pub fn current_state(&self) -> LockSurfaceState {
        compositor::with_states(&self.surface, |states| {
            states.data_map.get::<Mutex<LockSurfaceAttributes>>().unwrap().lock().unwrap().current
        })
    }
}
...
impl LockSurfaceConfigure {
    fn new(state: LockSurfaceState) -> Self {
        Self { serial: SERIAL_COUNTER.next_serial(), state }
    }
}
