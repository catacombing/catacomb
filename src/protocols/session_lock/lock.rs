...
use crate::protocols::session_lock::surface::{
...
pub struct SessionLockState {
    pub lock_status: Arc<AtomicBool>,
}

impl SessionLockState {
    pub fn new() -> Self {
        Self { lock_status: Arc::new(AtomicBool::new(false)) }
    }
}
    ...
    fn request(
        ...
        match request {
            Request::GetLockSurface { id, surface, output } => {
                ...
                let data = ExtLockSurfaceUserData { surface: surface.clone() };
                let shell_surface = data_init.init(id, data);

                // Initialize surface data.
                compositor::with_states(&surface, |states| {
                    states.data_map.insert_if_missing_threadsafe(|| {
                        Mutex::new(LockSurfaceAttributes::default())
                    })
                });

                // Add pre-commit hook for updating surface state.
                compositor::add_pre_commit_hook(&surface, |_data: &mut (), _dh, surface| {
                    compositor::with_states(surface, |states| {
